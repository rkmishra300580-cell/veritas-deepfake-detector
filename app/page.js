...
function ScanningPanel({ fileName, progress }) {
  return (
    <div style={{
      ...
      <div style={{ color: '#8b949e', fontSize: 14, marginBottom: 24 }}>{fileName}</div>
      <div style={{ maxWidth: 320, margin: '0 auto' }}>
        ...
        <div style={{ fontFamily: 'monospace', fontSize: 11, color: '#586069', marginTop: 8 }}>
...
export default function DeepfakeDetectorApp() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0e14', color: '#e6edf3', fontFamily: "'Inter', -apple-system, sans-serif" }}>
      <style>{`* { box-sizing: border-box; } ::selection { background: #00d4d4; color: #0a0e14; }`}</style>

      <header style={{ borderBottom: '1px solid #1f2937', padding: '18px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <ShieldAlert size={22} color="#00d4d4" />
          <span style={{ fontWeight: 700, fontSize: 17, letterSpacing: -0.3 }}>VERITAS</span>
          <span style={{ fontFamily: 'monospace', fontSize: 11, color: '#586069', border: '1px solid #1f2937', padding: '2px 6px', borderRadius: 3 }}>FORENSIC ENGINE v5</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, fontSize: 13, color: '#8b949e' }}>
          <span>Pro Plan</span>
          <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#1f2937', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600 }}>U</div>
        </div>
      </header>
      ...
      <main style={{ maxWidth: 920, margin: '0 auto', padding: '48px 24px' }}>
        ...
        {stage === 'upload' && (
          <div>
            ...
            <div onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}
              ...
              style={{ border: `2px dashed ${dragActive ? '#00d4d4' : '#1f2937'}`, borderRadius: 6, padding: '56px 24px', textAlign: 'center', cursor: 'pointer', background: dragActive ? 'rgba(0,212,212,0.04)' : '#0d1117', transition: 'all 0.15s ease' }}>
            ...
            {file && (
              <div style={{ marginTop: 20, display: 'flex', gap: 12, alignItems: 'center' }}>
                <button onClick={startAnalysis} style={{ background: '#00d4d4', color: '#0a0e14', border: 'none', borderRadius: 5, padding: '12px 24px', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
                  Run analysis <ChevronRight size={16} />
                </button>
                <button onClick={() => setFile(null)} style={{ background: 'transparent', color: '#8b949e', border: '1px solid #1f2937', borderRadius: 5, padding: '12px 18px', fontSize: 14, cursor: 'pointer' }}>Clear</button>
              </div>
            )}
            <div style={{ marginTop: 48, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
              {[{ icon: FileImage, label: 'Images', desc: 'Frequency, face & DL analysis' }, { icon: FileVideo, label: 'Video', desc: 'Frame & temporal consistency' }, { icon: FileAudio, label: 'Audio', desc: 'Voice clone / TTS detection' }, { icon: FileText, label: 'Documents', desc: 'AI-generated text detection' }].map((item) => (
        ...
        {stage === 'error' && (
          <div style={{ background: '#0d1117', border: '1px solid #ef4444', borderRadius: 6, padding: 32, textAlign: 'center' }}>
            ...
            <button onClick={reset} style={{ background: '#1f2937', color: '#e6edf3', border: 'none', borderRadius: 5, padding: '10px 20px', fontSize: 14, cursor: 'pointer' }}>Try another file</button>
          </div>
        )}

        {stage === 'results' && result && (
          <div>
            <div style={{ background: THREAT_CONFIG[result.threat_level]?.bg, border: `1px solid ${THREAT_CONFIG[result.threat_level]?.color}`, borderRadius: 6, padding: 32, marginBottom: 24, display: 'flex', gap: 32, alignItems: 'center', flexWrap: 'wrap' }}>
              <ScoreGauge score={result.final_score} threatLevel={result.threat_level} />
              <div style={{ flex: 1, minWidth: 240 }}>
                <div style={{ display: 'inline-block', fontSize: 11, fontWeight: 700, letterSpacing: 1, color: THREAT_CONFIG[result.threat_level]?.color, border: `1px solid ${THREAT_CONFIG[result.threat_level]?.color}`, borderRadius: 3, padding: '3px 10px', marginBottom: 12, textTransform: 'uppercase' }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                  <button onClick={downloadPDF} style={{ background: '#00d4d4', color: '#0a0e14', border: 'none', borderRadius: 5, padding: '9px 16px', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Download size={14} /> Download full PDF report
                  </button>
                  <button onClick={reset} style={{ background: 'transparent', color: '#8b949e', border: '1px solid #1f2937', borderRadius: 5, padding: '9px 16px', fontSize: 13, cursor: 'pointer' }}>Analyze another file</button>
                </div>
              </div>
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: '#0d1117', border: '1px solid #1f2937', borderRadius: 5, marginBottom: 24, fontFamily: 'monospace', fontSize: 12, color: '#8b949e' }}>
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div style={{ background: '#0d1117', border: '1px solid #1f2937', borderRadius: 5, overflow: 'hidden' }}>
                  {result.indicators.map((ind, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px', fontSize: 13, borderBottom: i < result.indicators.length - 1 ? '1px solid #1f2937' : 'none' }}>
            ...
            <div>
              ...
              <div style={{ background: '#0d1117', border: '1px solid #1f2937', borderRadius: 5, padding: 16, fontFamily: 'monospace', fontSize: 12, color: '#8b949e', lineHeight: 2 }}>
